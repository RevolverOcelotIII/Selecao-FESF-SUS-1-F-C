...
from app.core.auth import get_current_user, require_pharma_scope, require_admin
...
router = APIRouter(prefix="/medications", tags=["Medications"], dependencies=[Depends(get_current_user)])
...
@router.post("/", response_model=MedicationResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_pharma_scope)])
...
@router.put("/{medication_id}", response_model=MedicationResponse, dependencies=[Depends(require_pharma_scope)])
...
@router.delete("/{medication_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_admin)])
